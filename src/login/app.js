...
/* ============================================================
   Harmonix — Sign in, 100% inline styles.
   Same visual language as the register page: glass card over a
   drifting concert backdrop, floating glows, hover/focus states
   and the spinner all driven by React state + requestAnimationFrame
   instead of CSS classes, keyframes, or media queries.
   ============================================================ */
...
export default function HarmonixLoginInline() {
  ...
  return (
    ...
    >
      ...
      >
        {submitted ? (
          <SuccessState email={form.email} onReset={() => setSubmitted(false)} />
        ) : (
          <>
            <div style={{ textAlign: "center", marginBottom: 26 }}>
              ...
              <p style={{ fontSize: 13.5, color: COLORS.muted, margin: 0 }}>
                Sign in to keep the rhythm going
              </p>
            </div>
            ...
            <form style={{ display: "flex", flexDirection: "column", gap: 18 }} onSubmit={handleSubmit}>
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: "#d8d6e2" }}>Password</label>
                  <TextLink small>Forgot password?</TextLink>
                </div>
                <FieldBox focused={focusField === "password"}>
                  <span style={{ color: focusField === "password" ? COLORS.purpleLight : "#8b889c", display: "flex" }}>
                    <Lock size={16} />
                  </span>
                  <input
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", fontSize: 13, color: "#c9c7d6" }}>
                ...
                <span
                  style={{
                    ...
                    height: 18,
                    borderRadius: 5,
                    border: `1px solid ${remember ? "transparent" : "#4a4760"}`,
                    flexShrink: 0,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    color: remember ? "#fff" : "transparent",
                    background: remember ? `linear-gradient(90deg, ${COLORS.purple}, ${COLORS.pink})` : "transparent",
            ...
            <p style={{ textAlign: "center", fontSize: 13, color: COLORS.muted, margin: "22px 0 0" }}>
              New to Harmonix? <TextLink>Create an account</TextLink>
            </p>
          </>
        )}
      </div>
      ...
      {!submitted && (
        <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", gap: 10, marginTop: 24, color: COLORS.muted, fontSize: 13 }}>
          ...
          <span>50k+ fans are already in</span>
        </div>
      )}
    </div>
  );
...
function FieldBox({ focused, error, children }) {
  return (
    <div
      style={{
        display: "flex",
...
function Field({ label, icon, trailing, focused, error, ...inputProps }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <FieldBox focused={focused} error={error}>
...
function TextLink({ children, small }) {
  const [hover, setHover] = useState(false);
  return (
    <a
      href="#"
      ...
      onMouseLeave={() => setHover(false)}
      style={{
        color: small ? COLORS.muted : COLORS.cyan,
        fontSize: small ? 12 : "inherit",
        fontWeight: small ? 600 : "inherit",
...
function SuccessState({ email, onReset }) {
  ...
  return (
    <div style={{ textAlign: "center", padding: "20px 0" }}>
      ...
      <h2 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 8px" }}>Welcome back!</h2>
      <p style={{ fontSize: 13.5, color: COLORS.muted, margin: "0 0 22px" }}>
        Signed in as <strong style={{ color: "#fff" }}>{email}</strong>. Redirecting to your dashboard...
      </p>
      <button
        type="button"
        onClick={onReset}
